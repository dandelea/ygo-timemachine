<template>
  <label :id="id" class="custom-label flex cursor-pointer">
    <div class="bg-white shadow w-6 h-6 p-1 flex justify-center items-center">
      <input type="checkbox" class="hidden" :checked="value" 
        @change="$emit('input', $event.target.checked)"
      >
      <svg class="hidden w-4 h-4 bg-blue-500 text-white pointer-events-none" viewBox="0 0 172 172">
    ...
    <span class="select-none flex flex-wrap items-center text-center text-sm">
      <slot></slot>
    </span>
  </label>
</template>
